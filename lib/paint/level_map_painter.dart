...
class LevelMapPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int currentLevel = 0; currentLevel < params.levelCount; currentLevel++) {
      final Offset p1 = Offset(_centerWidth, -(currentLevel * params.levelHeight));
      final Offset p2 =
          getP2OffsetBasedOnCurveSide(currentLevel, _p2_dx_VariationFactor, _p2_dy_VariationFactor, _centerWidth);
      final Offset p3 = Offset(_centerWidth, -((currentLevel * params.levelHeight) + params.levelHeight));

      _drawBezierCurve(canvas, p1, p2, p3, currentLevel + 1);

      if (params.enableVariationBetweenCurves) {
        _p2_dx_VariationFactor =
            _p2_dx_VariationFactor + params.curveReferenceOffsetVariationForEachLevel[currentLevel].dx;
        _p2_dy_VariationFactor =
            _p2_dy_VariationFactor + params.curveReferenceOffsetVariationForEachLevel[currentLevel].dy;
      }
    }
  ...
  Offset getP2OffsetBasedOnCurveSide(
      int currentLevel, double p2_dx_VariationFactor, double p2_dy_VariationFactor, double centerWidth) {
    final double clamped_dxFactor = p2_dx_VariationFactor.clamp(
        params.minReferencePositionOffsetFactor.dx, params.maxReferencePositionOffsetFactor.dx);
    final double clamped_dyFactor = p2_dy_VariationFactor.clamp(
        params.minReferencePositionOffsetFactor.dy, params.maxReferencePositionOffsetFactor.dy);
    final double p2_dx =
        currentLevel.isEven ? centerWidth * (1 - clamped_dxFactor) : centerWidth + (centerWidth * clamped_dxFactor);
    final double p2_dy = -((currentLevel * params.levelHeight) +
        (params.levelHeight * (currentLevel.isEven ? clamped_dyFactor : 1 - clamped_dyFactor)));
    return Offset(p2_dx, p2_dy);
  }

  void _drawBezierCurve(Canvas canvas, Offset p1, Offset p2, Offset p3, int currentLevel) {
    ...
    for (double t = _dashFactor; t <= 1; t += _dashFactor * 2) {
      ...
      Offset offset2 =
          Offset(_compute(t + _dashFactor, p1.dx, p2.dx, p3.dx), _compute(t + _dashFactor, p1.dy, p2.dy, p3.dy));
